...
$id=$_SESSION['UID'];
$link=mysqli_connect("localhost","root","","Professor_db");
$qry6="select distinct send_to_name,send_to_id from chat where send_from_id='$id'";
$resultset6=mysqli_query($link,$qry6);
	while($row=mysqli_fetch_row($resultset6)){
			echo "<h5><a href=chathistory.php?sendtoid=".$row[1].">".$row[0]."</a>"."</h5><br>";
	}
?>	
            </div>
		</div>
